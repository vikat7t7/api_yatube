...
def comment_1_post(post, user):
    ...
    return Comment.objects.create(author=user, post=post, text='Комментарий 1')


@pytest.fixture
def comment_2_post(post, another_user):
    from posts.models import Comment
    return Comment.objects.create(author=another_user, post=post, text='Комментарий 2')
...
def comment_1_another_post(another_post, user):
    from posts.models import Comment
    return Comment.objects.create(author=user, post=another_post, text='Комментарий 12')
